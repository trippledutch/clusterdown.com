...
for f in $(find . -name "*.html" -not -path "./.git/*"); do
  perl -0pi -e "s{(huisstijl/pagina\.js)(\?v=[0-9a-f]+)?}{\$1?v=$paginajs}g; \
                ...
                s{(huisstijl/storing\.css)(\?v=[0-9a-f]+)?}{\$1?v=$storing}g" "$f"
done
echo "pagina.css=$pagina pagina.js=$paginajs clustertriage.css=$ct clustertriage-v2.css=$ct2 storing.css=$storing"
